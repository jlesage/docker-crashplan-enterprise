...
This project provides a Docker container for [CrashPlan Enterprise](https://www.crashplan.com).
...
> [!NOTE]
> This Docker container is entirely unofficial and not made by the creators of CrashPlan Enterprise.
...
```shell
docker stop crashplan-enterprise
```

  2. Remove the container:
```shell
docker rm crashplan-enterprise
```
...
services:
  crashplan-enterprise:
    image: jlesage/crashplan-enterprise
    ports:
      - "5800:5800"
    volumes:
...
[Releases]: https://github.com/jlesage/docker-crashplan-enterprise/releases
[Docker Hub]: https://hub.docker.com/r/jlesage/crashplan-enterprise/tags
...
```shell
docker pull jlesage/crashplan-enterprise
```
...
For Synology NAS users, follow these steps to update a container image:
  ...
  3.  In the search bar, type the name of the container (`jlesage/crashplan-enterprise`).
  4.  Select the image, click *Download*, and choose the `latest` tag.
  5.  Wait for the download to complete. A notification will appear once done.
  6.  Click *Container* in the left pane.
  7.  Select your CrashPlan Enterprise container.
...
container, it would proxy all HTTP requests for `crashplan-enterprise.domain.tld` to
...
upstream docker-crashplan-enterprise {
...
server {
	...
	server_name crashplan-enterprise.domain.tld;

	location / {
	        proxy_pass http://docker-crashplan-enterprise;
	}
...
server {
	...
	location = /crashplan-enterprise {return 301 $scheme://$http_host/crashplan-enterprise/;}
	location /crashplan-enterprise/ {
		proxy_pass http://docker-crashplan-enterprise/;
		# Uncomment the following line if your Nginx server runs on a port that
		# differs from the one seen by external clients.
		#port_in_redirect off;
		location /crashplan-enterprise/websockify {
			proxy_pass http://docker-crashplan-enterprise/websockify;
			proxy_http_version 1.1;
			proxy_set_header Upgrade $http_upgrade;
			proxy_set_header Connection $connection_upgrade;
			proxy_read_timeout 86400;
		...
		# Needed when audio support is enabled.
		location /crashplan-enterprise/websockify-audio {
			proxy_pass http://docker-crashplan-enterprise/websockify-audio;
			proxy_http_version 1.1;
			proxy_set_header Upgrade $http_upgrade;
			proxy_set_header Connection $connection_upgrade;
			proxy_read_timeout 86400;
...
| **Application version** label | Displays the version of CrashPlan Enterprise integrated into Docker image. |
...
Any desktop notification generated by CrashPlan Enterprise is forwarded to the
...
the cache of CrashPlan can help resolve the issue:
  ...
  - Remove all the content of the `cache` directory found under the container's
    ...
    container is mapped to `/docker/appdata/crashplan-enterprise` on the host, the
